...
export const Drag = () => {
    ...
    const [selectedOption, setSelectedOption] = useState(localStorage.getItem('selectedOption') || 'detect_people'); // Default selection
    ...
    const handleDrop = async (e) => {
        ...
        if (!allowedExtensions.exec(file.name)) {
            ...
            try {
                const response = await fetch(`http://localhost:5000/${selectedOption}`, { // Using selectedOption
                    method: 'POST',
                    body: formData
                });
                ...
                setTimeout(() => {
                    if (response.status === 202) {
                        setFail(true);
                        toast({
                            title: "No se ha detectado ninguno de sus objetivos en la imagen.",
                            status: "error",
                            duration: 5000,
                            position: "top",
                            isClosable: true,
    ...
    return (
        <div className='drag'>
            ...
            <Select value={selectedOption} onChange={handleOptionChange} id='select'>
                    <option value="detect_people">Detectar personas</option>
